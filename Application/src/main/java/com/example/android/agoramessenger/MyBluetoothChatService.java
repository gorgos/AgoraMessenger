...
//        mHandler.sendMessage(msg);
    ...
    private class AcceptThread extends Thread {
        ...
        public void run() {
            ...
            while (mState != STATE_CONNECTED) {
                try {
                    if (socket == null) {
                        Log.e(TAG, "socket == null, stop BluetoothChatService!");
                        return;
                    }
    ...
    private class ConnectedThread extends Thread {
        ...
        public ConnectedThread(BluetoothSocket socket, String socketType) {
            ...
            protocolHelper = new ProtocolHelper(MainActivity.context, mHandler);
            ...
            // send all messages for device with end message
            List<Message> messages = protocolHelper.getMessagesForDevice("");
            for (Message m : messages) {
                String cmd = protocolHelper.buildMessageCmd(m);
                write(cmd.getBytes());
            }
            String noMoreToSend_cmd = protocolHelper.buildNoMoreToSendCmd();
            write(noMoreToSend_cmd.getBytes());
        }
